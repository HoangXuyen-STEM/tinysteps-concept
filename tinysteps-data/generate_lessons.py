...
def get_topic_content(lvl, topic_id, order, teacher, student, parent, vocab_items, grammar_items):
    ...
    name_a = random.choice(CHAR_NAMES.get(role_a, [teacher]))
    name_b_pool = [n for n in CHAR_NAMES.get(role_b, [student]) if n != name_a]
    name_b = random.choice(name_b_pool) if name_b_pool else student
...
def update_html_portal(level, lessons, data_dir):
    ...
    # --- Inject lesson data ---
    data_json = json.dumps(lessons, indent=2, ensure_ascii=False)
...
def update_index_html(data_dir):
    """Cập nhật số lượng bài học hoàn thành trên trang chủ index.html."""
    index_path = os.path.join(data_dir, "exports/index.html")
    ...
    # Map các thay thế mong muốn cho level-desc
    replacements = {
        'id="btn-starters"': ('<div class="level-desc">5/30 lessons mẫu</div>', '<div class="level-desc">30/30 lessons</div>'),
        'id="btn-movers"': ('<div class="level-desc">Overview & Syllabus</div>', '<div class="level-desc">30/30 lessons</div>'),
        'id="btn-flyers"': ('<div class="level-desc">Overview & Syllabus</div>', '<div class="level-desc">40/40 lessons</div>'),
        'id="btn-ket"': ('<div class="level-desc">Overview & Syllabus</div>', '<div class="level-desc">35/35 lessons</div>'),
        'id="btn-pet"': ('<div class="level-desc">Overview & Syllabus</div>', '<div class="level-desc">40/40 lessons</div>'),
    }

    modified = False
    for btn_id, (old_tag, new_tag) in replacements.items():
        # Tìm vị trí nút
        btn_pos = html.find(btn_id)
        if btn_pos != -1:
            # Tìm thẻ level-desc tiếp theo sau nút đó
            tag_pos = html.find(old_tag, btn_pos)
            if tag_pos != -1 and tag_pos < btn_pos + 300: # giới hạn phạm vi tìm kiếm trong cùng thẻ a
                html = html[:tag_pos] + new_tag + html[tag_pos + len(old_tag):]
                modified = True

    total_words = 0
    total_grammar_points = 0
    for level in ["starters", "movers", "flyers", "ket", "pet"]:
        with open(os.path.join(data_dir, f"vocabulary/{level}.json"), "r", encoding="utf-8") as f:
            total_words += len(json.load(f).get("words", []))
        with open(os.path.join(data_dir, f"grammar/{level}.json"), "r", encoding="utf-8") as f:
            total_grammar_points += len(json.load(f).get("grammar_points", []))

    word_stat_pattern = re.compile(
        r'(<div class="stat-card daily">\s*<div class="stat-value">)([^<]+)(</div>\s*<div class="stat-label">Rich Sample Dataset</div>)',
        re.MULTILINE,
    )
    grammar_stat_pattern = re.compile(
        r'(<div class="stat-card daily">\s*<div class="stat-value">)([^<]+)(</div>\s*<div class="stat-label">Grammar Patterns</div>)',
        re.MULTILINE,
    )

    html, word_subs = word_stat_pattern.subn(
        rf'\g<1>{total_words} Words\g<3>',
        html,
        count=1,
    )
    html, grammar_subs = grammar_stat_pattern.subn(
        rf'\g<1>{total_grammar_points} Points\g<3>',
        html,
        count=1,
    )
    modified = modified or bool(word_subs or grammar_subs)

    if modified:
        with open(index_path, "w", encoding="utf-8") as f:
            f.write(html)
        print("  [OK] exports/index.html updated with current lesson, vocabulary, and grammar counts")

def format_grammar_title(name):
    """Chuyển snake_case grammar names thành nhãn dễ đọc cho UI."""
    return name.replace("_", " ").title()

def update_vocabulary_overview_html(data_dir):
    """Cập nhật vocabulary-overview.html bằng toàn bộ vocabulary và grammar registry."""
    overview_path = os.path.join(data_dir, "exports/vocabulary-overview.html")
    ...
    for level in levels:
        ...
        with open(os.path.join(data_dir, f"grammar/{level}.json"), "r", encoding="utf-8") as f:
            grammar_points = json.load(f).get("grammar_points", [])

        vocabulary_data[level] = [
            {
                "id": word["id"],
                "word": word["word"],
                "ipa": word.get("ipa", ""),
                ...
                "image_hint": word.get("image_hint", ""),
            }
            for word in words
        ]
        grammar_data[level] = [
            {
                "id": point["id"],
                "title": format_grammar_title(point.get("name", point["id"])),
        ...
        total_points += len(grammar_points)

    with open(overview_path, "r", encoding="utf-8") as f:
        html = f.read()

    vocab_start = html.find("const vocabularyData = {")
    grammar_start = html.find("const grammarData = {", vocab_start)
    switch_start = html.find("\n    function switchLevel(", grammar_start)

    if vocab_start == -1 or grammar_start == -1 or switch_start == -1:
        print("  [WARNING] Không tìm thấy markers trong vocabulary-overview.html")
        return

    vocabulary_block = f"const vocabularyData = {json.dumps(vocabulary_data, indent=2, ensure_ascii=False)};\n\n"
    grammar_block = f"    const grammarData = {json.dumps(grammar_data, indent=2, ensure_ascii=False)};"

    html = html[:vocab_start] + vocabulary_block + grammar_block + html[switch_start:]
    html = html.replace(
        "Bản ghi toàn bộ từ vựng và cấu trúc ngữ pháp thuộc mẫu thử nghiệm 5 levels. Lọc dữ liệu bằng bộ lọc để tiến hành đánh giá chi tiết.",
        f"Bản ghi đầy đủ {total_words} từ vựng và {total_points} cấu trúc ngữ pháp trên 5 levels. Lọc dữ liệu bằng bộ lọc để tiến hành đánh giá chi tiết.",
    )

    with open(overview_path, "w", encoding="utf-8") as f:
        f.write(html)
    print(f"  [OK] {overview_path} → {total_words} words, {total_points} grammar points")

def select_lesson_vocab(level_vocab, matching_vocab, lesson_size, rotation_offset, topical_quota=4):
    """Ưu tiên từ đúng topic nhưng vẫn xoay đều toàn bộ pool của level."""
    if not level_vocab:
    ...
    for item in matching_vocab[:min(topical_quota, lesson_size)]:
        if item["id"] in selected_ids:
            continue
        selected.append(item)
        selected_ids.add(item["id"])

    pool_size = len(level_vocab)
    scan_index = 0
    while len(selected) < min(lesson_size, pool_size) and scan_index < pool_size:
        candidate = level_vocab[(rotation_offset + scan_index) % pool_size]
        if candidate["id"] not in selected_ids:
            selected.append(candidate)
            selected_ids.add(candidate["id"])
        scan_index += 1

    next_offset = (rotation_offset + scan_index) % pool_size
...
def main():
    ...
    students = ["Nam", "Lan", "An", "Vy", "Duy", "Khang", "Phong", "Trang"]
    ...
    for lvl in levels:
        ...
        start_order  = 1
        lesson_count = target_count - start_order + 1
        topical_quota = 4
        lesson_vocab_size = min(
            len(lvl_vocab),
            max(8, ((len(lvl_vocab) + lesson_count - 1) // lesson_count) + topical_quota),
        )
        vocab_rotation_offset = 0
        ...
        for order in range(start_order, target_count + 1):
            ...
            lesson_vocab, vocab_rotation_offset = select_lesson_vocab(
                lvl_vocab,
                matching_vocab,
                lesson_vocab_size,
                vocab_rotation_offset,
                topical_quota,
            )
            ...
            char_teacher = random.choice(teachers)
            char_student = random.choice(students)
            char_parent  = random.choice(parents)
    ...
    update_index_html(DATA_DIR)
    update_vocabulary_overview_html(DATA_DIR)
