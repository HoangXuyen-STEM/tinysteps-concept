...
import { getListeningLevelProgress } from "@/lib/progress/listening-progress-queries";
import { hasPaidAccess, isFreeListening } from "@/lib/access/paid-access";
...
export default async function ListeningLevelPage({ params }: { params: Promise<{ level: string }> }) {
  ...
  const exercises = getListeningByLevel(level);
  const progress = await getListeningLevelProgress(level);
  const isPaid = await hasPaidAccess();

  return (
    <main>
      <h1 className="text-2xl font-bold capitalize text-slate-900">{labels.LISTENING} · {level}</h1>
      <p className="mt-2 text-slate-600">Nghe tối đa 3 lần rồi điền, đạt ít nhất 80% câu đúng.</p>
      <div className="mt-6 space-y-3">
        {exercises.map((exercise) => {
          const itemProgress = progress[exercise.id];
          const isLocked = !isPaid && !isFreeListening(exercise.id);
          return (
            <Link
              ...
              href={`/listening/${level}/${exercise.id}`}
              key={exercise.id}
            >
